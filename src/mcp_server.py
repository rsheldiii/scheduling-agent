...
# ---------------------------------------------------------------------------
# LLM-powered prepare: scenario selection, question generation, call brief
# ---------------------------------------------------------------------------


class _PrepareResult(BaseModel):
    scenario: str
    questions: list[str]
    call_instructions: str


async def _llm_prepare(user_prompt: str) -> _PrepareResult:
    available = _prompt_loader.list(category="outgoing")
    scenarios_text = "\n".join(
...
"""
    ...
    agent: Agent[_PrepareResult] = Agent(
        name="PrepareAnalyzer",
        instructions=instructions,
        output_type=_PrepareResult,
    )
    result = await Runner.run(agent, input=user_prompt)
    analysis = result.final_output  # type: ignore[return-value]
    logger.info(
...
async def prepare_call(user_prompt: str) -> dict:
    ...
    analysis = await _llm_prepare(user_prompt)
